...
export class PdfviewComponent  implements OnInit{
  ...
  generatePDF1(invoiceData: any) {
    ...
    const columns = ["Sr", "Product", "Ch No", "HSN No", "Pieces", "Cut", "Quantity", "Pricce/Unit", "Amount"];
    ...
    for (let i = 0; i < 10; i++) {
      const bodyRows = [
        ...
        data[i]?.chalanNo ? data[i]?.chalanNo.toString() : '',
        data[i]?.hsnNumber ? data[i]?.hsnNumber.toString() : '',
        data[i]?.pieces ? data[i]?.pieces.toString() : '',
        data[i]?.cut ? Number(data[i]?.cut).toFixed(2).toString() : '',
        data[i]?.qty ? Number(data[i]?.qty).toFixed(2).toString() : '',
        data[i]?.priceUnit ? Number(data[i]?.priceUnit).toFixed(2).toString() : '',
    ...
    body.push(
      ...
      [{ content: `${finalAmountInWords}`, rowSpan: 3, colSpan: 7, styles: { halign: 'center', fontStyle: 'bold' } }, `SGST ${invoiceData.sGST}%`, `Rs. ${sGstAmountFormatted}`],
  ...
  submitInvoice() {
    const productsSubTotal = this.invoiceData.products.reduce((acc: any, product: any) =>
      acc + product.finalAmount, 0);
    const discountAmount = (productsSubTotal * (this.invoiceData.discount / 100));
    const discountedSubTotal = productsSubTotal - discountAmount;
    const sGstAmount = discountedSubTotal * (this.invoiceData.sGST / 100);
    const cGstAmount = discountedSubTotal * (this.invoiceData.cGST / 100);
    const finalAmount = discountedSubTotal + sGstAmount + cGstAmount;

    const roundedAmount = Math.round(finalAmount);
    this.invoiceData.formattedRoundedAmount = new Intl.NumberFormat('en-IN').format(roundedAmount);
    this.invoiceData.finalAmountInWords = this.toWords.convert(Number(roundedAmount));
